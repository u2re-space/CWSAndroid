...
class ReverseGatewayClient(
    ...
    private fun describeCandidateLabel(candidate: WebsocketCandidate): String {
        val endpoint = candidate.endpoint.trim()
        return when {
            endpoint.startsWith("ws://") || endpoint.startsWith("wss://") -> endpoint
            endpoint.startsWith("http://") && candidate.scheme == "ws" -> endpoint.replaceFirst("http://", "ws://")
            endpoint.startsWith("http://") && candidate.scheme == "wss" -> endpoint.replaceFirst("http://", "wss://")
            endpoint.startsWith("https://") && candidate.scheme == "ws" -> endpoint.replaceFirst("https://", "ws://")
            endpoint.startsWith("https://") && candidate.scheme == "wss" -> endpoint.replaceFirst("https://", "wss://")
            else -> "${candidate.scheme}://$endpoint"
        }
    }
