...
fun SettingsScreen(
    ...
    fun refreshDaemonSnapshot() {
        ...
        if (daemon != null) {
            daemonSnapshot = daemon.getConnectionSnapshot()
            return
        }
        val preview = previewEndpointConfig()
        val endpoint = preview.endpointUrl.ifBlank { preview.dispatchUrl }
        val candidateList = preview.endpointCandidates.joinToString(", ").ifBlank {
            preview.endpointUrl.ifBlank { preview.dispatchUrl }
        }
        daemonSnapshot = if (preview.isRemoteReady()) {
            DaemonConnectionSnapshot.inactiveConfigured(
                endpoint = endpoint,
                configured = true,
                enabled = true,
                stateDetail = "daemon inactive in app process",
                candidateList = candidateList
            )
        } else {
            DaemonConnectionSnapshot.stopped()
        }
    }
