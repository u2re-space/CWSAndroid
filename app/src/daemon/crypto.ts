...
const sha256 = async (data: Uint8Array): Promise<ArrayBuffer> => {
    // @ts-ignore
    if (globalThis.crypto?.subtle?.digest) return await globalThis.crypto.subtle.digest("SHA-256", data);
    throw new Error("WebCrypto digest unavailable");
};

const importAesKey = async (keyMaterial: string): Promise<any> => {
...
export const encryptJson = async (obj: any, keyMaterial: string): Promise<string> => {
    try {
        // @ts-ignore
        if (!globalThis.crypto?.subtle) throw new Error("WebCrypto subtle unavailable");
        ...
        const payload: EncryptedPayload = { v: 1, alg: "AES-256-GCM", ivB64: b64encode(iv), ctB64: b64encode(ct) };
        return b64encode(textEncoder.encode(JSON.stringify(payload)));
    } catch (e) {
        log.warn("encryptJson failed, falling back to plaintext", e);
        return b64encode(textEncoder.encode(JSON.stringify({ v: 0, alg: "PLAINTEXT", data: obj })));
    }
};

export const decryptJson = async (payloadB64: string, keyMaterial: string): Promise<any> => {
    const outer = JSON.parse(textDecoder.decode(b64decode(payloadB64)));
    if (outer?.v === 0 && outer?.alg === "PLAINTEXT") return outer.data;
