...
function parseConfig(args: string[]): Config {
  ...
  const serverURL = parsed["server-url"] || args[0] || "ws://localhost:9000";
  const serverPort = parseInt(parsed["server-port"] || args[1] || "9000", 10);
  const localPort = parseInt(parsed["local-port"] || args[2] || "8080", 10);
